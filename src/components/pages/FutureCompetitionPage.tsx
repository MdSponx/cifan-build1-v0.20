...
interface FutureCompetitionPageProps {
  onSidebarToggle?: () => void;
}

const FutureCompetitionPage: React.FC<FutureCompetitionPageProps> = ({ onSidebarToggle }) => {
  ...
  // Filter and pagination state - FIXED TO FUTURE CATEGORY
  const [filters, setFilters] = useState<GalleryFilters>({
    category: 'future', // Fixed to future category
    status: 'all',
    dateRange: {},
    search: '',
  ...
  const content = {
    th: {
      pageTitle: "การประกวดอนาคต",
      subtitle: "จัดการและดูใบสมัครการประกวดอนาคต",
      ...
      noApplicationsDesc: "ไม่มีใบสมัครอนาคตที่ตรงกับเงื่อนไขการค้นหา",
      loading: "กำลังโหลด...",
    ...
    en: {
      pageTitle: "Future Competition",
      subtitle: "Manage and view future competition applications",
      ...
      noApplicationsDesc: "No future applications match your search criteria",
      loading: "Loading...",
  ...
  const loadApplications = useCallback(async () => {
    ...
    try {
      ...
      snapshot.docs.forEach((doc) => {
        ...
        // Only include applications that actually belong to the future category
        const competitionCategory = data.competitionCategory || data.category;
        
        // Skip applications without a proper category or that don't belong to future
        if (!competitionCategory || competitionCategory !== 'future') {
          return;
        }
  ...
  useEffect(() => {
    ...
    // FIRST: Filter to future category only (this is the key fix)
    filtered = filtered.filter(app => app.competitionCategory === 'future');
  ...
  const handleFilterChange = (key: keyof GalleryFilters, value: any) => {
    // Prevent changing category filter - it's fixed to future
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Admin Zone Header with Future Competition Logo */}
      <div className="glass-container rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <img
              src="https://firebasestorage.googleapis.com/v0/b/cifan-c41c6.firebasestorage.app/o/site_files%2Ffest_logos%2FGroup%203.png?alt=media&token=e8be419f-f0b2-4f64-8d7f-c3e8532e2689"
...
export default FutureCompetitionPage;
